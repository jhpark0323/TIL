...
def is_validation(user_data_dict):
    # 각각을 변수로 받음
    blood_group = user_data_dict['blood_group']
    company = user_data_dict['company']
    mail = user_data_dict['mail']
    name = user_data_dict['name']
    website = user_data_dict['website']
...
# is_validation(user_data[0])
# print(wrong)

def create_user(ls):
    global wrong_num
    for user in ls:
        if is_validation(user) == False:
            user[wrong[0]] = None
            wrong_num += 1
            user_list.append(user)

        elif is_validation(user) == True:
            user_list.append(user)

        elif is_validation(user)[0] == False:
            for i in range(len(wrong)):
                user[wrong[i]] = None
            wrong_num += 1
            user_list.append(user)

        elif is_validation(user) == 'bloked':
            wrong_num += 1
